...
def searching():
    ...
    return render_template('pages/searching.html', query=query, last_years=last_years)
...
def long_task():
    ...
    return jsonify({}), 202, {'Location': url_for('pages.task_status',
                                                  task_id=task_id)}
    ...
    if task.state == 'PENDING':
        ...
        if 'result' in task.info:
            ...
            response['url_for'] = url_for(
                'pages.search', query=task.info['query'], last_years=task.info['last_years'])
    else:
...
def home():
    ...
    return render_template('pages/home.html', stories_examples=stories_examples, form=form, lang_code=lang_code)
...
def search():
    ...
    if 'id' in request.args:
        ...
        try:
            ...
            print('Invalid task')
            return render_template('pages/search.html', lang_code=lang_code, form=SearchForm(), related_terms=[], result=None, providers=news_domains, user_query=None, hasNarrative=hasNarrative)
        
        if not result:
            print('Invalid result')
            return render_template('pages/search.html', lang_code=lang_code, form=form, related_terms=[], result=None, providers=news_domains, user_query=fquery, hasNarrative=hasNarrative)
            ...
            if(result["status"] != "OK"):
                return render_template('pages/search.html', lang_code=lang_code, form=form, related_terms=[], result=None, result_header=result_header, providers=news_domains, hasNarrative=hasNarrative)
        ...
        if 'query' in request.args:
            return redirect(url_for('pages.searching', query=fquery, last_years=flast_years))
